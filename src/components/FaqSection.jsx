...
const FaqSection = () => {
    ...
    return (
        <section
            ref={sectionRef}
            className={`site-section py-20 transition-all duration-700 md:translate-y-8 ${isInView ? 'translate-y-0 opacity-100' : 'md:opacity-0'} lg:py-36 lg:pb-24 lg:pt-16`}
            id="faq"
        >
                    ...
                    <div className="relative z-2 flex w-full flex-wrap justify-between">
                        <header className="mb-7 w-full lg:w-5/12">
                            <h6 className="relative mb-12 inline-flex translate-y-6 items-center gap-1.5 overflow-hidden rounded-full bg-secondary px-5 py-2 text-secondary-foreground shadow-xs shadow-primary">
                                ...
                                </svg> الأسئلة المتداولة
                            </h6>
                            <h2 className="mb-7 text-white">
                                هل لديك سؤال؟
                            </h2>
                            <p className="m-0 text-white/70">
                                "فريق الدعم لدينا جاهز لمساعدتك في أي استفسار يتعلق بإنشاء وإدارة متجرك الإلكتروني. نقدم إرشادات مخصصة لضمان نجاح أعمالك بكل سهولة."
                            </p>
                        </header>

                        <div className="lqd-accordion flex w-full flex-col gap-7 lg:w-6/12">
                            {faqItems.map((item) => (
                                <div key={item.id} className="lqd-accordion-item group relative rounded-lg bg-black px-7">
                                    <button
                                        onClick={() => handleAccordionClick(item.id)}
                                        className={`group/btn peer flex w-full items-center justify-between py-7 text-start text-xl font-normal ${activeAccordion === item.id ? 'lqd-is-active' : ''}`}
                                    >
                                    ...
                                    <span className={`lqd-outline-glow absolute inline-block rounded-[inherit] pointer-events-none overflow-hidden lqd-outline-glow-effect-1 transition-opacity ${activeAccordion === item.id ? 'opacity-100' : 'opacity-0'}`}>
                                    ...
                                    <div className={activeAccordion === item.id ? 'block' : 'hidden'}>
